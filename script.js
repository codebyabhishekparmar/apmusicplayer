...
async function getsongs(){
    let a = await fetch("http://127.0.0.1:3000/song/");//get data from the server (we are using await taki sare song ek sath aa jai)
    ...
    for (let i = 0; i < as.length; i++){
        if(as[i].href.endsWith(".mp3")){  //Checks if the href (the link) ends with .mp3 — meaning it's an MP3 file.
            ...
            songs.push(as[i].href.split("/song/")[1]);
        }    
    }            
    // console.log(songs) //direct print karne pr songs ek sath aa jate hai na ke one by one is liye return. due to this list of songs will be created 
    return songs;
} 
...
function playmusic(track) {
    ...
    currentsong.src= "/song/" + track; // to play one song at a time 
    currentsong.play();
    ...
    let  a =  track.replaceAll("%20"," ").replaceAll("(Mp3 Song)"," ").replaceAll("(SambalpuriStar.In)"," ").replaceAll("-"," ").replaceAll(".mp3"," "); 
...
async function main(){   
    ...
    console.log(songs);
     ...
     // Set the first song as the default song
...
// hamko songs me currentsong.src ko search karna hai pr currentsong.src me puri link aati hai or song list me (name of song). toh hamko currentsong.src ko songs me search karne k liye slice karna padega  
      ...
      /*http://127.0.0.1:3000/spotify_clone/song/Kendrick%20Lamar,%20SZA%20-%20All%20The%20Stars.mp3*/
       ...
       /*"Kendrick%20Lamar,%20SZA%20-%20All%20The%20Stars.mp3",
        ...
        console.log(currentsong.src.split("/song/")[1]);
        /*"Kendrick%20Lamar,%20SZA%20-%20All%20The%20Stars.mp3"*/
